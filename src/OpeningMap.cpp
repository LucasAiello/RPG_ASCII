...
void OpeningMap::init()
{
    ...
    house = new ObjetoDeJogo("House", Sprite("../rsc/sprite_house.spr"), 7, 20);
    objs.push_back(house);
    colisons.push_back(house);
	
	fence = new ObjetoDeJogo("Fence", SpriteAnimado("../rsc/sprite_fence.anm", 3), 7, 74);
	objs.push_back(fence);
    colisons.push_back(fence);
	
	sign = new ObjetoDeJogo("Sign", Sprite("../rsc/sprite_sign.spr"), 18, 170);
	objs.push_back(sign);
	...
	west_entrance = new Texts(ObjetoDeJogo("West Entrance", Sprite("../rsc/sprite_vertical_entrance.spr"), 24, 0), {"PRECISO CURAR A VILA ANTES DE IR EMBORA..."});
...
{	
    ...
    while (true)
	{
		...
		{
			if (player->colideCom(*house_text)) PlaySound(TEXT("../rsc/knock.wav"), NULL, SND_FILENAME | SND_ASYNC);
			showText(posL, posC);
		}
